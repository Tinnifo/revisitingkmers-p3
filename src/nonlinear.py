...
def run(
    ...
    if use_wandb:
        if wandb is None:
            raise ImportError(
                "Weights & Biases is not installed but --use_wandb was specified."
            )
        wandb_kwargs = wandb_kwargs or {}
        wandb_config = wandb_config or {}
        wandb_run = wandb.init(**wandb_kwargs, config=wandb_config)
    ...
    for epoch in range(epoch_num):
        ...
        if wandb_run is not None:
            wandb_run.log({"loss/train": avg_loss, "epoch": epoch + 1})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Weights & Biases entity/user",
    )
    parser.add_argument(
        "--wandb_run_name",
        type=str,
    ...
    effective_batch_size = args.batch_size if args.batch_size else len(training_dataset)
